// dsh-cross-platform/test/witness-final-linux-test.ts —— Witness 12 项验收 Linux 版（bash 化 + bwrap 沙箱）
// 对齐 Windows 版 12 项结构；命令 bash 化；D 组用 bwrap 只读视图（EXP-3）
...
const BASE = fs.mkdtempSync(path.join(os.tmpdir(), 'wfinal-lx-'))
let rootSeq = 0
const mkRoot = () => {
...
// Linux 版任务进程定位：runner 的子进程（pgrep -P）
...
console.log('  Witness 12 项验收 · Linux 版（bash + bwrap 沙箱）')
console.log('='.repeat(72))
...
// ===== D-01 防覆盖（Linux：bwrap 只读视图） =====
{
  const root = mkRoot()
  const r1 = reg(root)
  const id = r1.start({ kind: 'bash', label: 'd01', command: "echo ORIGINAL; sleep 2; echo OVERWRITE > out.log 2>/dev/null && echo OVERWROTE || echo OVERWRITE-BLOCKED", sandbox: 'bwrap' } as never)
  await r1.wait(id, 60000)
...
// ===== D-02 防删（Linux：bwrap 只读视图 + exit.txt +i） =====
{
  const root = mkRoot()
  const r1 = reg(root)
  const id = r1.start({ kind: 'bash', label: 'd02', command: "echo KEEP-ME; sleep 2; rm -f out.log 2>/dev/null && echo OUT-DELETED || echo OUT-PROTECTED; rm -f exit.txt 2>/dev/null && echo EXIT-DELETED || echo EXIT-PROTECTED", sandbox: 'bwrap' } as never)
  await r1.wait(id, 60000)
  const outFile = path.join(root.jobsRoot, id, 'out.log')
  let statOk = true
